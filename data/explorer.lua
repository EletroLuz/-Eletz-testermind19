...
local start_time = 0
local min_duration = 15 -- Minimum 5 seconds -- Tempo minimo duracao explorer
...
on_update(function()
    if enabled then
        if os.clock() - start_time < min_duration then
            ...
            if calculate_distance(get_player_position(), target_position) < 2 then
                disable()
                _G.explorer_active = false
            else
                -- Continue exploring if not complete
                check_walkable_area()
                local is_stuck = check_if_stuck()
